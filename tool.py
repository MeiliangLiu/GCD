...
def var_read(sparsity, lag, trial):
    data = sio.loadmat(
        r'E:\efficient-kan\VAR\sparsity=' + str(sparsity) + ', lag=' + str(lag) + '\data\sim' + str(
            trial) + '.mat')
    GC = sio.loadmat(r'E:\efficient-kan\VAR\sparsity=' + str(sparsity) + ', lag=' + str(lag) +
...
def fmri_read(simulation, subject):
    data = sio.loadmat(r'E:\efficient-kan\fmri bold\sim' + str(simulation) + '.mat')
